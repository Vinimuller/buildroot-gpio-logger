...
Esta aplicação realiza a leitura periódica do estado de determinadas GPIOs e grava esse estado em arquivo de log no sistema. O foco é oferecer uma solução leve para monitoramento de pinos digitais em sistemas embarcados com libgpiod, especialmente integrados via Buildroot. Aplicação conta com um arquivo de configuração que possibilita nomear os pinos da GPIO
...
**S98gpio-logger** → Script de inicialização colocado em /etc/init.d/ para rodar automaticamente no boot.